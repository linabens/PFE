...
router.post('/', authenticateSession, orderCreateRateLimiter, async (req, res) => {
  ...
  try {
    ...
    console.log('Incoming order request:', { bodyTableId, items_count: items?.length });
    ...
    console.log('Parsed table_id:', table_id);

    if (!table_id || isNaN(table_id)) {
      console.error('Invalid table_id:', table_id);
      return res.status(400).json({ success: false, error: 'table_id est requis et doit être un nombre' });
    }
