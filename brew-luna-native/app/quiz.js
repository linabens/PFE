...
export default function QuizGameScreen() {
  ...
  if (screen === 'result') {
    ...
    const medal = pct >= 80 ? { icon: '🏆', label: 'Légendaire !',   color: C.primary }
                : pct >= 50 ? { icon: '🌟', label: 'Bien joué !',   color: C.accent }
                :             { icon: '🎯', label: 'Bel effort !',   color: C.muted };
    return (
      <SafeAreaView style={s.safe}>
        <StatusBar barStyle="dark-content" backgroundColor={C.bg} />
        <ScrollView contentContainerStyle={[s.scroll, { alignItems: 'center' }]} showsVerticalScrollIndicator={false}>
          <Animated.View style={{ opacity: fadeAnim, alignItems: 'center', width: '100%' }}>
            <Text style={s.medal}>{medal.icon}</Text>
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.topBar}>
        ...
        <View style={s.streakBadge}>
          <Text style={s.streakTxt}>{streak > 0 ? `🔥 ×${streak}` : level?.emoji}</Text>
        </View>
      </View>
        ...
        <Animated.View style={[s.qHeader, { opacity: fadeAnim }]}>
          <View style={s.catBadgeInline}>
            <Text style={s.catBadgeInlineTxt}>
              {category?.emoji} {category?.label.toUpperCase()} · {level?.label}
            </Text>
          </View>
...
const s = StyleSheet.create({
  ...
  // Result
  medal:       { fontSize: 80, marginTop: 40, marginBottom: 10 },
  ...
  streakBadge:{ backgroundColor: C.card, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: C.border },
  ...
  catBadgeInline:  { backgroundColor: C.card, borderRadius: 12, paddingHorizontal: 12, paddingVertical: 6, borderWidth: 1, borderColor: C.border },
