...
import { View, Text, StyleSheet, TouchableOpacity, StatusBar, ScrollView, Animated, Dimensions, Platform, ActivityIndicator } from 'react-native';
    ...
    if (currentOrderId) {
      const fetchStatus = async () => {
        try {
          ...
          // Stop polling if we see auth/session issues to avoid spamming the logs
          if (e.message.includes('403') || e.message.includes('401') || e.message.includes('session')) {
...
const styles = StyleSheet.create({
  ...
  mainBadge: { alignSelf: 'center', backgroundColor: C.mainText, paddingHorizontal: 20, paddingVertical: 10, borderRadius: 20, borderWidth: 1, borderColor: C.rosewood, marginBottom: 40 },
