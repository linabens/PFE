...
import { View, Text, StyleSheet, TouchableOpacity, Image, FlatList, TextInput, Animated, Easing, Platform, Dimensions, ScrollView } from 'react-native';
...
export default function HomeScreen() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (catsRes.success && prodsRes.success) {
        setCategories([{ id: 'all', name: 'Tous', emoji: '☕' }, ...catsRes.data]);
  ...
  const confirmCallStaff = async () => {
    ...
    try {
      const res = await assistanceApi.requestAssistance(1); 
      if (res.success) {
        setShowCallModal(false);
        showToast("Un serveur arrive ! 🛎️");
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={[styles.hBadge, { backgroundColor: C.primary }]} onPress={() => router.push('/loyalty')}>
        ...
        ListHeaderComponent={
          <View style={styles.contentPadding}>
            ...
            <View style={styles.greetingRow}>
              <Text style={styles.greetingTitle}>{new Date().getHours() < 18 ? 'Bonjour ☀️' : 'Bonsoir 🌙'}</Text>
                  ...
                  {tendancesList.map((tProduct, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <View style={styles.tStars}><Text style={{ fontSize: 10, color: C.rosewood }}>★★★★★</Text></View>
            ...
            {specialsList.length > 0 && (
              <View style={styles.sectionBlock}>
                <Text style={[styles.sectionTitle, { marginBottom: 16 }]}>☀️ Spécial du jour</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginHorizontal: -16, paddingHorizontal: 16 }}>
      ...
      {/* FAB: Call Staff */}
      <TouchableOpacity style={styles.fabStaff} onPress={handleCallStaff} activeOpacity={0.8}>
...
const styles = StyleSheet.create({
  ...
  hBadge: { backgroundColor: C.border, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  hBadgeText: { color: C.primary, fontFamily: FONT.poppins, fontSize: 11, fontWeight: 'bold' },
  logoBlock: { alignItems: 'center' },
  logoFont: { fontFamily: FONT.playfair, fontSize: 22, fontWeight: '600' },
  logoTagline: { fontFamily: FONT.poppins, fontSize: 9, letterSpacing: 1.5, color: C.tagline, marginTop: -2 },
  ...
  greetingSession: { fontFamily: FONT.poppins, fontSize: 10, color: C.secondaryText, paddingBottom: 2 },
  ...
  sectionTitle: { fontFamily: FONT.playfair, fontSize: 16, color: C.primary, fontWeight: 'bold' },
  sectionLink: { fontFamily: FONT.poppins, fontSize: 11, color: C.rosewood, fontWeight: '600' },
  ...
  pBadgeText: { color: C.bg, fontFamily: FONT.poppins, fontSize: 8, fontWeight: 'bold' },
  ...
  priceValue: { fontFamily: FONT.poppins, fontSize: 14, fontWeight: 'bold', color: C.mainText },
  ...
  toastContainer: { position: 'absolute', bottom: 20, alignSelf: 'center', backgroundColor: C.mainText, paddingHorizontal: 18, paddingVertical: 8, borderRadius: 20, zIndex: 60 },
  toastText: { color: C.bg, fontFamily: FONT.poppins, fontSize: 11 }
});
