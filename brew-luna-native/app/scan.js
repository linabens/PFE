...
export default function QRScanScreen() {
  ...
  const authenticateTable = async (tableValue) => {
    try {
      // Use the real API to create a session in the backend
      ...
      if (res.success) {
        ...
        alert(res.error || 'Erreur lors de la création de la session');
      }
    } catch (e) {
  ...
  const handleBarCodeScanned = async ({ type, data }) => {
    ...
    // If it's a real QR from our system, it might be a code like 'QR-TABLE-001'
    // Let's try to use scanQrCode first
    try {
      const res = await sessionApi.scanQrCode(data);
      if (res.success) {
        setSession({
          token: res.data.token,
          table_id: res.data.table_id,
      ...
      // Fallback to manual extraction if scan fails
    }

    // Extraction logic for fallback
    let extractedId = data;
    try {
      if (data.includes('table=')) {
        extractedId = data.split('table=')[1].split('&')[0];
      }
    } catch { }
