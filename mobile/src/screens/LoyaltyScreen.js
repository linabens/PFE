    ...
    <SafeAreaView style={s.safe}>
      ...
      <Animated.View style={[s.container, { opacity: fadeAnim }]}>
        ...
        <View style={s.heroSection}>
          ...
          <Text style={s.title}>Coffee Time Rewards</Text>
          <Text style={s.subtitle}>Gagnez des points à chaque gorgée</Text>

          {/* Séparateur décoratif */}
          <View style={s.divider}>
            <View style={s.dividerLine} />
            <Moon size={16} color={C.muted} strokeWidth={1.75} />
            <View style={s.dividerLine} />
          </View>
        </View>
        ...
        <View style={s.tiersPreview}>
          {[
            ...
            { name: 'Silver', color: '#8B8FA8' },
            { name: 'Gold', color: '#B8860B' },
            { name: 'Platinum', color: '#6B3A2A' },
          ].map((tier) => (
            <View key={tier.name} style={s.tierPill}>
              <TierBadgeIcon tierName={tier.name} size={14} color={tier.color} />
